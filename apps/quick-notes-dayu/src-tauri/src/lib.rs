...
fn load_store(app: AppHandle) -> Result<QuickNotesStore, String> {
    ...
    let contents =
        fs::read_to_string(&store_path).map_err(|error| format!("读取本地数据失败：{error}"))?;
...
pub fn run() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![
            ...
            save_store,
            toggle_devtools,
            worklog::load_worklog_config,
            worklog::save_worklog_config,
            worklog::validate_repository,
            worklog::scan_git_commits,
            worklog::validate_zentao_config,
            worklog::list_zentao_projects,
            worklog::list_zentao_executions,
            worklog::preview_zentao_tasks,
            worklog::create_and_finish_zentao_tasks
        ])
