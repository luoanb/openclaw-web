<script lang="ts">
  ...
  import WorklogTab from "$lib/features/worklog/WorklogTab.svelte";
  ...
  <header class="flex h-14 shrink-0 items-center gap-4 border-b px-4">
    <div class="flex min-w-0 items-center gap-4">
      ...
      <nav class="flex h-14 items-end gap-1" aria-label={t("tab.tasks")}>
        ...
        <button
          class="h-14 border-b-2 px-3 text-sm font-medium transition-colors hover:text-foreground"
          class:border-foreground={activeTab === "worklog"}
          class:text-foreground={activeTab === "worklog"}
          class:border-transparent={activeTab !== "worklog"}
          class:text-muted-foreground={activeTab !== "worklog"}
          type="button"
          aria-current={activeTab === "worklog" ? "page" : undefined}
          onclick={() => selectTab("worklog")}
        >
          {t("tab.worklog")}
        </button>
      </nav>
    ...
    <div class="relative ml-auto w-72">
      ...
      <input
        ...
        placeholder={activeTab === "worklog" ? t("search.worklogPlaceholder") : t("search.placeholder")}
      />
    </div>
