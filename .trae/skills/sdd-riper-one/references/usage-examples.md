...
- 产物文件示例：`docs/specs/YYYY-MM-DD_hh-mm_资源访问审批与时效控制.md`。
...
- 写入 `docs/codemap/` 下的功能级地图文件。
- 产物文件示例：`docs/codemap/YYYY-MM-DD_hh-mm_访问审批链路功能.md`。
...
- 产物文件示例：`docs/codemap/YYYY-MM-DD_hh-mm_sample-enterprise项目总图.md`。
...
```text
build_context_bundle: ./docs/context/raw/external-share
```
...
- 产物文件示例：`docs/context/YYYY-MM-DD_hh-mm_资源访问审批与时效控制_context_bundle.md`。
...
```text
验证功能: log_path=./logs/release-test/, spec=docs/specs/2026-02-27_14-30_资源访问审批与时效控制.md
```
...
```text
REVIEW SPEC: spec=docs/specs/2026-02-27_14-30_资源访问审批与时效控制.md, scope=plan_only
```
...
```text
REVIEW EXECUTE: spec=docs/specs/2026-02-27_14-30_资源访问审批与时效控制.md, scope=changed_only
```
...
```text
ARCHIVE: targets=[docs/specs/2026-02-27_14-30_资源访问审批与时效控制.md,docs/codemap/2026-02-27_13-50_访问审批链路功能.md], kind=mixed, audience=both, mode=snapshot, topic=访问审批时效化
```
...
- 生成双文档：
  - `docs/archive/YYYY-MM-DD_hh-mm_访问审批时效化_human.md`
  - `docs/archive/YYYY-MM-DD_hh-mm_访问审批时效化_llm.md`
- 两个文档都包含 `Trace to Sources`（结论 -> 来源文件/章节）。

用户输入（主题归档）：

```text
主题归档: targets=./docs/specs/, kind=spec, audience=llm, mode=thematic, topic=审批域授权策略演进
```
