...
- 方案摘要：
  - 将服务预览拆成三层：Discovery / Selection / Render。
  - `packages/os-core` 只定义第一层 Discovery：可预览服务地址 target 的事实模型、registry、发现事件和通用错误形态。
  - 在 `packages/browserpod` 新增 BrowserPod preview discovery adapter，主路径监听 `pod.onPortal(({ url, port }) => ...)`，将 Portal 转为统一 `PreviewTarget`。
  - 在 `apps/web-claw` 内部实现第二层 Selection 和第三层 Render：当前选中 target、iframe 加载状态、刷新、外部打开、清除和手动 URL 输入。
...
- 为什么选择该方案：
  ...
  - `os-core` 只沉淀跨 runtime 稳定事实，保持 app 不直接认识 BrowserPod SDK，同时避免把 web UI 的选择态和 iframe 生命周期污染到公共 core。
...
- 不采用的方案：
  ...
  - 不把 `selected`、`loading`、`ready`、`failed` 这类 UI/view 状态写入 `os-core` target 模型。
...
- `packages/os-core/src/preview/*`：
  - 只定义服务地址发现层的通用契约、target 事实模型、registry 事件、错误形态和 URL 规范化工具。
  - 不引用 BrowserPod、DOM iframe 或 Svelte。
- `packages/browserpod/src/preview/*`：
  - 实现 BrowserPod preview discovery adapter。
  - 封装 `pod.onPortal`、Portal URL 去重、session stale 防护和 BrowserPod 错误映射。
- `apps/web-claw/src/lib/core/preview/*`：
  - 管理应用内 selection state 与 render state。
  - 连接 runtime session、preview target registry 和 UI view model。
...
- `PreviewTarget` 是服务地址事实，不表达用户当前是否选中，也不表达 iframe 是否 loading / ready / failed。
...
- `PreviewTargetRegistry` 只维护当前 runtime session 下的 target 列表与去重。
- `addTarget` 供 discovery adapter 和 app 手动 URL 入口写入 target；它不选择、不加载、不探活。
- `removeTarget` / `clearTargets` 只删除 target 事实；如果被删除 target 正在被 UI 选中，由 app selection state 处理清空或切换。
- `PreviewTargetDiscoveryService.attach(...)` 只绑定已运行的 runtime session；容器未运行时由 app 层展示阻塞态，不创建 discovery attachment。
- `PreviewDiscoveryAttachment.close()` 清理 discovery listener；BrowserPod `onPortal` 无 unsubscribe 时，adapter 通过 closed flag 忽略后续 stale callback。
...
- target 排序在 `os-core` 中只保证可预测：按 `discoveredAt` 倒序或插入顺序稳定返回；“当前选中优先、最近打开优先”等属于 app selection/view 排序。
- 同一个 `url` 或同一个 `port + url` 重复触发时更新已有 target 的 `discoveredAt` / metadata，不新增重复项。
...
- iframe 相关错误不进入 `os-core` discovery 层，放在 `apps/web-claw` 的 render state 中。

### App 内 Selection / Render 状态
...
- `PreviewSelectionState` 表达用户想看哪个 target。
- `PreviewRenderState` 表达当前渲染载体的加载结果；第一阶段载体是 iframe + 外部打开。
- `iframe-load-failed`、`iframe-blocked` 等错误属于 render state，不进入 `os-core` target 模型。
- app 可按 `selectedTargetId`、`lastSelectedAt`、`render.updatedAt` 对 target 列表做用户视角排序。
...
- `BrowserPodPreviewDiscoveryService` 依赖 `BrowserPodRuntimeManager.resolvePod(runtimeSession)`，与 terminal service 当前模式一致。
- 若 `typeof pod.onPortal !== "function"`，返回结构化 `portal-api-unavailable`，并保持 Preview Tab 可手动输入 URL。

### Portal 事件处理

```text
BrowserPodPreviewDiscoveryService.attach(runtimeSession, registry)
  -> resolvePod(runtimeSession)
  -> assert onPortal function
  -> create PreviewDiscoveryAttachment
  -> pod.onPortal(({ url, port }) => registry.addTarget({ url, port, source: "portal" }))
```

- `handlePortal(url, port)`：
  - 校验 `url` 是合法 `http:` / `https:`。
  - 以 `url` 为主键去重；同一 URL 重复出现时更新已有 entry。
  - 生成 target：`source: "portal"`、`label: Port ${port}`。
  - 不选中 target，不设置 iframe 状态，不直接触发 UI 切换。
  - 发出 `preview-target-added` 或 `preview-target-updated`；app 可基于事件显示轻量 toast 或 Tab badge。
- 因官方 `onPortal` 不返回 unsubscribe：
  - discovery attachment 持有 `closed` flag。
  - callback 进入时先检查当前 session id 和 closed flag。
  - runtime stop 或 discovery attachment close 后忽略旧 callback。
...
- `PreviewServiceProvider`：
  - 项目级单例，创建 BrowserPod preview discovery service。
  - 只在 provider 内依赖 `BrowserPodRuntimeManager`；展示组件只依赖 app state。
- `PreviewWorkspaceState`：
  - class，持有 `PreviewTargetRegistrySnapshot`、`PreviewSelectionState`、`PreviewRenderState` 和 manual input 状态。
  - 订阅 runtime manager：runtime 从 `running` 变为非 running 时关闭 discovery attachment，并清理 selection/render 状态。
  - runtime 为 `running` 且有 currentSession 时创建 target registry，并 attach discovery service。
  - 监听 target registry 事件：第一个 target 可由 app 自动选中；后续 target 只加入列表和提示，不抢占当前 render。
...
- Toolbar：
  ...
  - `Clear` 清除当前 target；`Clear all` 可放入更多菜单。
- iframe viewport：
  - 选中 target 后设置 iframe `src`。
  - `onload` 后更新 app 内 `PreviewRenderState.status = "ready"`。
  - load 超时或 error 时更新 app 内 render failed；仍保留外部打开。
...
- 发现 Portal：
  ...
  - 不自动切换 Tab。
- 多地址：
  - 第一个 target 可由 app selection state 自动选中。
  - 后续 target 只加入列表，不抢占当前 iframe。
- 手动 URL：
  - 成功添加后选中该 entry。
...
- 后续若要扩展 `terminal-output` 来源，必须先：
  ...
  - 在 `os-core` preview 中提供 `PreviewTargetUrlDetector`，只处理纯文本 chunk。
...
- 包契约：
  - 新增或重构 `packages/os-core/src/preview/preview.interfaces.ts`，只保留 `PreviewTarget*` / registry / discovery 契约。
  - 新增或重构 `packages/os-core/src/preview/preview.errors.ts`，只保留 discovery 层错误。
  - 新增 `packages/os-core/src/preview/index.ts`
  - 更新 `packages/os-core/src/index.ts`
- BrowserPod adapter：
  - 新增或重构 `packages/browserpod/src/preview/browserpodPreviewDiscovery.impl.ts`
...
1. 新增 / 重构 `os-core` preview discovery 契约：
   - 定义 `PreviewTarget`、`PreviewTargetRegistry`、`PreviewTargetDiscoveryService`、snapshot、event、error、action result。
   - 用 class 封装 target registry 状态管理，例如 `PreviewTargetRegistryState`。
   - 不引用 DOM、Svelte 或 BrowserPod。
   - 不包含 selected / loading / ready / failed 这类 app view 状态。
2. 实现 BrowserPod preview discovery adapter：
   - 收窄 `BrowserPodLike.onPortal` 类型。
   - `BrowserPodPreviewDiscoveryService.attach(runtimeSession, registry)` 解析 pod 并注册 `onPortal`。
   - 将 Portal URL 写入 registry，由 registry 去重并维护多个 target。
   - 无 `onPortal` 时返回结构化错误，允许 app 使用手动 URL。
3. 接入 app preview core：
   - 新增 provider 创建 preview discovery service。
   - 新增 `PreviewWorkspaceState` 订阅 runtime 状态，running 时创建 registry 并 attach discovery，非 running 时 close。
   - `PreviewWorkspaceState` 内维护 selection state 和 render state，不在 Svelte 组件中散落业务状态。
4. 实现 `PreviewPanel.svelte`：
   - 容器未运行：展示阻塞态和打开 runtime drawer 入口。
   - 无服务地址：展示空态和手动 URL 输入。
   - 有服务地址：展示列表、iframe、刷新、外部打开、清除。
   - iframe load/timeout/error 回写 app 内 render state。
5. 接入 `App.svelte`：
