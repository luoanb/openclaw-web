...
- 主体：
  - 首次或配置缺失时：展示初始化配置弹窗，完成禅道连接验证后才允许关闭。
...
- 结构：
  ...
  - 底部状态：验证失败原因或验证成功提示。
- 关闭规则：
  - 未验证成功时不提供关闭入口，或关闭按钮禁用并说明“验证成功后可关闭”。
  - `验证` 只负责验证和保存，不关闭弹窗。
  - 底部保留 `保存` 按钮；验证成功后启用，点击后保存当前初始化配置并关闭弹窗。
