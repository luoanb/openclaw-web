...
- 主体：
  - 左侧：同步配置与仓库列表。
  - 中间：指定日期的提交记录预览。
  - 右侧：禅道项目/迭代选择、任务创建与完成配置、执行确认、最近运行结果、定时触发。
- 底部或局部反馈：
  - 复用现有保存失败横条模式。
...
- 主体采用 3 栏布局：
  - 左栏宽度约 `320px`，用于仓库与基础配置。
  - 中栏自适应，最小宽度约 `420px`，用于提交预览。
  - 右栏宽度约 `360px`，用于禅道字段、确认提交和定时设置。
...
├────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ 仓库与日期                         │ 2026-07-16 的提交记录                         │ 禅道打卡         │
│                                    │                                                │                  │
│ 日期  [ 2026-07-16 ▾ ] [抓取提交]  │ 8 commits from 3 repositories                 │ API 已验证        │
│                                    │                                                │ https://zentao... │
│ 仓库                               │ □ openclaw-web              4 commits   成功   │                  │
│ [启用] openclaw-web                │   17:20 fix quick notes sync mapping           │ 项目     [可选 ▾] │
│       D:\work-space\openclaw-web   │   16:42 add WSL repository requirement         │ 迭代     [可选 ▾] │
│                                    │   11:08 polish task row copy                   │ 类型     [开发 ▾] │
│ [启用] dayu-api                    │                                                │ 预计工时 [ 1.0 ]  │
│       \\wsl$\Ubuntu\home\me\api    │ □ dayu-api                   3 commits   成功   │ 任务名模板        │
│                                    │   15:01 implement zentao client retry          │ [{date}] {repo}   │
│ [停用] legacy-admin                │   10:16 adjust worklog payload                 │                  │
│       E:\repos\legacy-admin        │                                                │ [预览打卡任务]    │
│                                    │ □ legacy-admin               已停用            │ [创建并完成]      │
│ + 添加仓库                         │                                                │                  │
│                                    │ 最近一次自动同步：今天 18:30，等待执行          │ 定时自动触发      │
│ 定时                               │                                                │ [开] 每天 18:30   │
│ [开] 每天 18:30                    │                                                │                  │
└────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
- 第一步：左栏选择日期并确认仓库启用状态。
- 第二步：点击 `抓取提交`，中栏进入抓取状态。
- 第三步：抓取完成后，中栏按仓库分组展示 commits，默认全选可提交记录。
- 第四步：右栏拉取并选择项目/迭代，配置任务类型、预计工时和任务名模板，点击 `预览打卡任务`。
- 第五步：预览确认后，点击 `创建并完成`，系统创建任务后立即完成该任务。
...
- 顶部状态：
  ...
  - `验证失败`：展示失败原因和重试入口。
- 字段区：
  - 顶部提供 `保存禅道配置` 操作，保存 API 地址、账号、项目/迭代、任务类型、工时、模板和定时相关默认值。
  - 项目：从禅道拉取候选后选择。
  - 迭代：从禅道拉取候选后选择。
...
- 操作区：
  - `预览打卡任务`：secondary。
  - `创建并完成`：primary，仅在有待创建任务且禅道配置有效时启用。
...
- 开启后展示：
  ...
  - 下一次运行时间。
  - 最近一次运行结果。
- 配置不完整时，Switch 可打开但显示 `配置未完成，不会打卡`；或直接禁用并说明原因，技术方案阶段二选一固化。
...
还没有抓取记录
选择日期并点击“抓取提交”，系统会从已启用仓库读取当天 commit。
