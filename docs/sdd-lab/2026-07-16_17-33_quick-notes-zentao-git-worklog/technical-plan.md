...
- `activeView: "init" | "clockIn"`：【禅道打卡】Tab 内当前 view；未验证时固定为 `init`，验证通过后可在两个 view 间切换。
...
- 改动类型：新增
- 改动内容：
  - 进入页面时，如果已保存禅道地址、账号和密码，则自动验证连接；验证成功进入打卡 view，验证失败进入初始化配置 view。
  - 实现初始化配置 view 与打卡 view 两个视图，不使用弹窗、蒙版或遮罩承载初始化门禁。
  - 初始化配置 view 标题为“初始化配置”；密码右侧提供“验证”按钮，负责验证连接与保存配置。
  - 验证成功后进入打卡 view；初始化完成后可在初始化配置 view 与打卡 view 之间任意切换。
  - 仓库配置可在初始化配置 view 内维护但不阻塞进入打卡 view。
...
- 风险：初始化配置被误做成局部或全局 Modal，造成阻塞范围和视觉语义混乱。
  - 缓解方式：初始化配置不使用 dialog/overlay，改为 `WorklogTab` 内的独立 view；打卡 view 只在禅道验证通过后可进入。
